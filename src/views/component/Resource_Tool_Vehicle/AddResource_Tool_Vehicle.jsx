...
const AddResource_Tool_Vehicle = () => {
      ...
      <div className="container my-5" data-bs-theme={darkMode ? 'dark' : 'light'}>
        <div className="card shadow">
          <div className="card-body p-5">
              <h3 className="fw-bold mb-5">Add Resource / Tool / Vehicle</h3>
                  ...
                  <select name="assignedTo" className="form-select" required>
                    ...
                    <optgroup label="Departments">
                      {departments.map(dept => (
                        <option key={`dept-${dept.id}`} value={JSON.stringify({ id: dept.id, label: `${dept.nameD} (Dept)` })}>
                          {dept.nameD} (Dept)
                        </option>
                      ))}
                    </optgroup>
                    ...
                    <optgroup label="NGOs">
                      {ngo.map(ngo => (
                        <option key={`ngo-${ngo.id}`} value={JSON.stringify({ id: ngo.id, label: `${ngo.nameN} (NGO)` })}>
                          {ngo.nameN} (NGO)
                        </option>
                      ))}
                    </optgroup>
                    ...
                    {employees.map(emp => (
                      <option key={`emp-${emp.id}`} value={JSON.stringify({ id: emp.id, label: `${emp.user_name} (User)` })}>
                        {emp.user_name} (User)
                      </option>
                    ))}
                  </optgroup> */}
...
export default AddResource_Tool_Vehicle;